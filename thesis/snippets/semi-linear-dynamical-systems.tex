...
\subsection{M-Step}
	...
	\begin{align*}
		...
		\mat{P}_{t \subgiven t_0}^{(n)}        & \coloneqq \E\bigg[\vec{s}_t^{(n)} \Big(\vec{s}_t^{(n)}\Big)^T \bigggiven \vec{y}_{1:t_0}\bigg]       & \mat{P}_{t \subgiven t_0}        & \coloneqq \frac{1}{N} \sum_{n = 1}^{N} \hat{\mat{P}}_{t \subgiven t_0}^{(n)}        \\
		\mat{P}_{t, t - 1 \subgiven t_0}^{(n)} & \coloneqq \E\bigg[\vec{s}_t^{(n)} \Big(\vec{s}_{t - 1}^{(n)}\Big)^T \bigggiven \vec{y}_{1:t_0}\bigg] & \mat{P}_{t, t - 1 \subgiven t_0} & \coloneqq \frac{1}{N} \sum_{n = 1}^{N} \hat{\mat{P}}_{t, t - 1 \subgiven t_0}^{(n)}
	\end{align*}
	...
	\end{align*}
	Finally, we drive \(Q_4\). This is the one that depends on the non-closed-form expectations~\eqref{eq:expectedMeasurement} and~\eqref{eq:expectedMeasurementMat}:
	\begin{align*}
		Q_4
	...
	\begin{align}
		...
		\implies && \mat{R} &= \frac{1}{NT} \sum_{n = 1}^{N} \sum_{t = 1}^{T} \vec{y}_t^{(n)} \Big(\vec{y}_t^{(n)}\Big)^T - \hat{\vec{g}}_t^{(n)} \Big(\vec{y}_t^{(n)}\Big)^T - \vec{y}_t^{(n)} \Big(\hat{\vec{g}}_t^{(n)}\Big)^T + \Big(\mat{G}_t^{(n)}\Big)^T & \label{eq:measurementNoiseCovariance}
	\end{align}
	...
	\begin{align}
		\int\! \vec{g}\Big(\vec{s}_t^{(n)}\Big) p\Big(\vec{s}_t^{(n)} \Biggiven \vec{y}_{1:T}\Big) \dd{\vec{s}_{1:T}^{(n)}}
			&\approx \sum_{i = 1}^{2k} \vec{g}\bigg(\!\sqrt{\mat{V}_t} \vec{\xi}_i + \hat{\vec{s}}_t^{(n)}\!\bigg)  \label{eq:lgsCubatureG} \\
		\int\! \vec{g}\Big(\vec{s}_t^{(n)}\Big) \Big(\vec{g}\Big(\vec{s}_t^{(n)}\Big)\Big)^T p\Big(\vec{s}_t^{(n)} \Biggiven \vec{y}_{1:T} \Big)
			&\approx \sum_{i = 1}^{2k} \vec{g}\bigg(\!\sqrt{\mat{V}_t} \vec{\xi}_i + \hat{\vec{s}}_t^{(n)}\!\bigg) \bigg(\!\vec{g}\bigg(\!\sqrt{\mat{V}_t} \vec{\xi}_i + \hat{\vec{s}}_t^{(n)}\!\bigg)\!\bigg)^T  \label{eq:lgsCubatureGG}
	\end{align}
	...
	\begin{align*}
		...
		\hat{\vec{g}}_t^{(n)} &\approx \sum_{i = 1}^{2k} \vec{g}\bigg(\!\sqrt{\mat{V}_t} \vec{\xi}_i + \hat{\vec{s}}_t^{(n)}\!\bigg) \\
		\mat{G}_t^{(n)} &\approx \sum_{i = 1}^{2k} \vec{g}\bigg(\!\sqrt{\mat{V}_t} \vec{\xi}_i + \hat{\vec{s}}_t^{(n)}\!\bigg) \bigg(\!\vec{g}\bigg(\!\sqrt{\mat{V}_t} \vec{\xi}_i + \hat{\vec{s}}_t^{(n)}\!\bigg)\!\bigg)^T
	\end{align*}
...
\subsection{E-Step}
	To get the expectations \( \hat{\vec{s}}_t \), \( \mat{P}_{t} \) and \( \mat{P}_{t, t - 1} \), we need to calculate the distribution \( p(\vec{s}_t \given \vec{y}_{1:T}) \) for each time step \(t\) (and, consequently, for each observation sequence \(n\)). This is exactly the posterior distribution calculated by a smoother. Thus we divide the E-step into two parts~\cite{minkaHiddenMarkovModels1999}:
	\begin{enumerate}
		\item Filtering: To calculate the posterior distribution \( p(\vec{s}_t \given \vec{y}_{1:t}) \), we employ a Gaussian filter similar to the standard Kalman filter.
		\item Smoothing: To calculate the smoothed posterior distribution \( p(\vec{s}_t \given \vec{y}_{1:T}) \), we employ a Gaussian \ac{rts} smoother.
	\end{enumerate}

	% TODO: Specify what that all s-hats refer to a single sequence...
	\subsubsection{Forward Pass}
		...
		\begin{align*}
			\hat{\vec{s}}_{t \subgiven t} & = \hat{\vec{s}}_{t \subgiven t - 1} + \mat{K}_{t \subgiven t - 1} (\mat{S}_{t \subgiven t - 1})^{-1} (\vec{y}_t - \hat{\vec{y}}_{t \subgiven t - 1}) \\
			\mat{V}_{t \subgiven t}       & = \mat{V}_{t \subgiven t - 1} - \mat{K}_{t \subgiven t - 1} (\mat{S}_{t \subgiven t - 1})^{-1} \mat{V}_{t \subgiven t - 1}
		\end{align*}
		with the expectations and covariances
		\begin{align}
			\hat{\vec{s}}_{t \subgiven t - 1} & \coloneqq \E_{\vec{s}_t}[\vec{s}_t \given \vec{y}_{1:t - 1}]                         \label{eq:filterPriorState}      \\
			\mat{V}_{t \subgiven t - 1}       & \coloneqq \Cov_{\vec{s}_t}[\vec{s}_t \given \vec{y}_{1:t - 1}]                       \label{eq:filterPriorCovariance} \\
			\hat{\vec{y}}_{t \subgiven t - 1} & \coloneqq \E_{\vec{y}_t}[\vec{y}_t \given \vec{y}_{1:t - 1}]                         \label{eq:filterPriorMeas}       \\
			\mat{S}_{t \subgiven t - 1}       & \coloneqq \Cov_{\vec{y}_t}[\vec{y}_t \given \vec{y}_{1:t - 1}]                       \label{eq:filterPriorMeasCov}    \\
			\mat{K}_{t \subgiven t - 1}       & \coloneqq \Cov_{\vec{s}_t, \vec{y}_t}[\vec{s}_t, \vec{y}_t \given \vec{y}_{1:t - 1}] \label{eq:filterPriorKalman}
		\end{align}
		...
		\begin{align*}
			\hat{\vec{s}}_{t \subgiven t - 1}
				&= \E_{\vec{s}_t}[\vec{s}_t \given \vec{y}_{1:t - 1}] \\
				&= \E_{\vec{s}_{t - 1}, \vec{w}_t}[\mat{A} \vec{s}_{t - 1} + \vec{w}_t \given \vec{y}_{1:t - 1}] \\
				&= \mat{A} \E_{\vec{s}_{t - 1}}[\vec{s}_{t - 1} \given \vec{y}_{1:t - 1}] \\
				&= \mat{A} \hat{\vec{s}}_{t - 1 \subgiven t - 1} \\
			\mat{V}_{t \subgiven t - 1}
				&= \Cov_{\vec{s}_t}[\vec{s}_t \given \vec{y}_{1:t - 1}] \\
				&= \Cov_{\vec{s}_{t - 1}}[\mat{A} \vec{s}_{t - 1} \given \vec{y}_{1:t - 1}] + \Cov_{\vec{w}_t}[\vec{w}_t \given \vec{y}_{1:t - 1}] \\
				&= \E_{\vec{s}_t}[\vec{s}_t \vec{s}_t^T \given \vec{y}_{1:t - 1}] + \mat{Q} \\
				&= \E_{\vec{s}_{t - 1}}[\mat{A} \vec{s}_{t - 1} \vec{s}_{t - 1} \mat{A}^T \given \vec{y}_{1:t - 1}] + \mat{Q} \\
				&= \mat{A} \E_{\vec{s}_{t - 1}}[\vec{s}_{t - 1} \vec{s}_{t - 1} \given \vec{y}_{1:t - 1}] \mat{A}^T + \mat{Q} \\
				&= \mat{A} \mat{V}_{t - 1 \subgiven t - 1} \mat{A}^T + \mat{Q}
		\end{align*}
		
		%By plugging in \( \vec{g}(\vec{s}_t) = \mat{C} \vec{s}_t \), \ac{ie} a purely linear system, we get exactly the Kalman filter equations:
		%\begin{align*}
		%	\hat{\vec{y}}_{t \subgiven t - 1} &= \mat{C} \hat{\vec{y}}_{t - 1 \subgiven t - 1} \\
		%	\mat{S}_{t \subgiven t - 1}
		%		&= \Cov_{\vec{y}_t}[\vec{y}_t \given \vec{y}_{1:t - 1}] \\
		%		&= \Cov_{\vec{s}_t}[\mat{C} \vec{s}_t \given \vec{y}_{1:t - 1}] + \mat{R} \\
		%		&= \mat{C} \E_{\vec{s}_t}[\vec{s}_t \vec{s}_t^T \given \vec{y}_{1:t - 1}] \mat{C}^T + \mat{R} \\
		%		&= \mat{C} \mat{V}_{t \subgiven t - 1} \mat{C}^T + \mat{R} \\
		%	\mat{K}_{t \subgiven t - 1} % TODO: This does not match the Kalman filter equation...
		%		&= \Cov_{\vec{s}_t, \vec{y}_t}[\vec{s}_t, \vec{y}_t \given \vec{y}_{1:t - 1}] \\
		%		&= \E_{\vec{s}_t}[\vec{s}_t \vec{s}_t^T \mat{C}^T \given \vec{y}_{1:t - 1}] - \hat{\vec{s}}_{t \subgiven t - 1} \hat{\vec{y}}_{t \subgiven t - 1}^T \\
		%		&= \E_{\vec{s}_t}[\vec{s}_t \vec{s}_t^T \given \vec{y}_{1:t - 1}] \mat{C}^T - \mat{A} \hat{\vec{s}}_{t \subgiven t - 1} \hat{\vec{s}}_{t - 1 \subgiven t - 1}^T \mat{C}^T \\
		%		&= \mat{V}_{t \subgiven t - 1} \mat{C}^T - \mat{A} \hat{\vec{s}}_{t - 1 \subgiven t - 1} \hat{\vec{s}}_{t \subgiven t - 1}^T \mat{C}^T
		%\end{align*}
		...
		\begin{align*}
			\hat{\vec{y}}_{t \subgiven t - 1}
				&= \E_{\vec{y}_t}[\vec{y}_t \given \vec{y}_{1:t - 1}] \\
				&= \E_{\vec{s}_t}[\vec{g}(\vec{s}_t) \given \vec{y}_{1:t - 1}] \\
				&= \int\! \vec{g}(\vec{s}_t) p(\vec{s}_t \given \vec{y}_{1:t - 1}) \dd{\vec{s}_t} \\
				&= \int\! \vec{g}(\vec{s}_t) \,\normal(\hat{\vec{s}}_{t \subgiven t - 1}, \mat{V}_{t \subgiven t - 1}) \dd{\vec{s}_t} \\
				&\approx \frac{1}{2k} \sum_{i = 1}^{2k} \vec{g}\big( \sqrt{\mat{V}_{t \subgiven t - 1}} \vec{\xi}_i + \hat{\vec{s}}_{t \subgiven t - 1} \big) \\
		\end{align*}
		\begin{align*}
			\mat{S}_{t \subgiven t - 1}
				&= \Cov_{\vec{y}_t}[\vec{y}_t \given \vec{y}_{1:t - 1}] \\
				&= \Cov_{\vec{s}_t}[\vec{g}(\vec{s}_t) \given \vec{y}_{1:t - 1}] \\
				&= \E_{\vec{s}_t}\bigg[\vec{g}(\vec{s}_t) \Big(\vec{g}(\vec{s}_t)\Big)^T \bigggiven \vec{y}_{1:t - 1}\bigg] \\
				&= \int\! \vec{g}(\vec{s}_t) \Big(\vec{g}(\vec{s}_t)\Big)^T p(\vec{s}_t \given \vec{y}_{1:t - 1}) \dd{\vec{s}_t} \\
				&= \int\! \vec{g}(\vec{s}_t) \Big(\vec{g}(\vec{s}_t)\Big)^T \,\normal(\hat{\vec{s}}_{t \subgiven t - 1}, \mat{V}_{t \subgiven t - 1}) \\
				&\approx \frac{1}{2k} \sum_{i = 1}^{2k} \vec{g}\bigg(\!\sqrt{\mat{V}_{t \subgiven t - 1}} \vec{\xi}_i + \hat{\vec{s}}_{t \subgiven t - 1}\!\bigg) \bigg(\!\vec{g}\bigg(\!\sqrt{\mat{V}_{t \subgiven t - 1}} \vec{\xi}_i + \hat{\vec{s}}_{t \subgiven t - 1}\!\bigg)\!\bigg)^T \\
		\end{align*}
		\begin{align*}
			\mat{K}_{t \subgiven t - 1}
				&= \Cov_{\vec{s}_t, \vec{y}_t}[\vec{s}_t, \vec{y}_t \given \vec{y}_{1:t - 1}] \\
				&= \E_{\vec{s}_t, \vec{y}_t}[\vec{s}_t \vec{y}_t^T \given \vec{y}_{1:t - 1}] - \E_{\vec{s}_t}[\vec{s}_t \given \vec{y}_{1:t - 1}] \big(\E_{\vec{y}_t}[\vec{y}_t \given \vec{y}_{1:t - 1}]\big)^T \\
				&= \E_{\vec{s}_t, \vec{y}_t}[\vec{s}_t \vec{y}_t^T \given \vec{y}_{1:t - 1}] - \hat{\vec{s}}_{t \subgiven t - 1} \hat{\vec{y}}_{t \subgiven t - 1}^T \\
				&= \E_{\vec{s}_t}[\vec{s}_t \Big(\vec{g}(\vec{s}_t)\Big)^T \given \vec{y}_{1:t - 1}] - \hat{\vec{s}}_{t \subgiven t - 1} \hat{\vec{y}}_{t \subgiven t - 1}^T \\
				&= \int\! \vec{s}_t \Big(\vec{g}(\vec{s}_t)\Big)^T p(\vec{s}_t \given \vec{y}_{1:t - 1}) \dd{\vec{s}_t} - \hat{\vec{s}}_{t \subgiven t - 1} \hat{\vec{y}}_{t \subgiven t - 1}^T \\
				&= \int\! \vec{s}_t \Big(\vec{g}(\vec{s}_t)\Big)^T \,\normal(\hat{\vec{s}}_{t \subgiven t - 1}, \mat{V}_{t \subgiven t - 1}) \dd{\vec{s}_t} - \hat{\vec{s}}_{t \subgiven t - 1} \hat{\vec{y}}_{t \subgiven t - 1}^T \\
				&\approx \frac{1}{2k} \sum_{i = 1}^{2k} \bigg(\!\sqrt{\mat{V}_{t \subgiven t - 1}} \vec{\xi}_i + \hat{\vec{s}}_{t \subgiven t - 1}\!\bigg) \bigg(\!\vec{g}\bigg(\!\sqrt{\mat{V}_{t \subgiven t - 1}} \vec{\xi}_i + \hat{\vec{s}}_{t \subgiven t - 1}\!\bigg)\!\bigg)^T - \hat{\vec{s}}_{t \subgiven t - 1} \hat{\vec{y}}_{t \subgiven t - 1}^T
		\end{align*}
		This completes the derivation of the forward pass equations.
		
		For completeness, we summarize all of them here:
		\begin{align*}
			\hat{\vec{s}}_{t \subgiven t - 1} & = \mat{A} \hat{\vec{s}}_{t - 1 \subgiven t - 1} \\
			\mat{V}_{t \subgiven t - 1}       & = \mat{A} \mat{V}_{t - 1 \subgiven t - 1} \mat{A}^T + \mat{Q} \\
			\hat{\vec{y}}_{t \subgiven t - 1} & \approx \frac{1}{2k} \sum_{i = 1}^{2k} \vec{g}\big( \sqrt{\mat{V}_{t \subgiven t - 1}} \vec{\xi}_i + \hat{\vec{s}}_{t \subgiven t - 1} \big)   \\
			\mat{S}_{t \subgiven t - 1}       & \approx \frac{1}{2k} \sum_{i = 1}^{2k} \vec{g}\bigg(\!\sqrt{\mat{V}_{t \subgiven t - 1}} \vec{\xi}_i + \hat{\vec{s}}_{t \subgiven t - 1}\!\bigg) \bigg(\!\vec{g}\bigg(\!\sqrt{\mat{V}_{t \subgiven t - 1}} \vec{\xi}_i + \hat{\vec{s}}_{t \subgiven t - 1}\!\bigg)\!\bigg)^T \\
			\mat{K}_{t \subgiven t - 1}       & \approx \frac{1}{2k} \sum_{i = 1}^{2k} \bigg(\!\sqrt{\mat{V}_{t \subgiven t - 1}} \vec{\xi}_i + \hat{\vec{s}}_{t \subgiven t - 1}\!\bigg) \bigg(\!\vec{g}\bigg(\!\sqrt{\mat{V}_{t \subgiven t - 1}} \vec{\xi}_i + \hat{\vec{s}}_{t \subgiven t - 1}\!\bigg)\!\bigg)^T - \hat{\vec{s}}_{t \subgiven t - 1} \hat{\vec{y}}_{t \subgiven t - 1}^T \\
			\hat{\vec{s}}_{t \subgiven t}     & = \hat{\vec{s}}_{t \subgiven t - 1} + \mat{K}_{t \subgiven t - 1} (\mat{S}_{t \subgiven t - 1})^{-1} (\vec{y}_t - \hat{\vec{y}}_{t \subgiven t - 1}) \\
			\mat{V}_{t \subgiven t}           & = \mat{V}_{t \subgiven t - 1} - \mat{K}_{t \subgiven t - 1} (\mat{S}_{t \subgiven t - 1})^{-1} \mat{V}_{t \subgiven t - 1}  
		\end{align*}
		The forward pass is initialized with \( \hat{\vec{s}}_{0 \subgiven 0} = \vec{m}_0 \) and \( \mat{V}_{0 \subgiven 0} = \mat{V}_0 \).
	% end
	...
	\subsubsection{Backward Pass}
		...
		\begin{align*}
			\hat{\vec{s}}_{t - 1 \subgiven T} & = \hat{\vec{s}}_{t - 1 \subgiven t - 1} + \mat{J}_{t - 1} (\hat{\vec{s}}_{t \subgiven T} - \hat{\vec{s}}_{t \subgiven t - 1}) \\
		...
		\begin{align}
			...
			\mat{V}_{t - 1, t \subgiven t - 1} &\coloneqq \Cov_{\vec{s}_{t - 1}, \vec{s}_t}[\vec{s}_{t - 1}, \vec{s}_t \given \vec{y}_{1:t - 1}]  \label{eq:smootherPriorCrossCov}
		\end{align}
		...
		\begin{align*}
			\mat{V}_{t - 1, t \subgiven t - 1}
				&= \Cov_{\vec{s}_{t - 1}, \vec{s}_t}[\vec{s}_{t - 1}, \vec{s}_t \given \vec{y}_{1:t - 1}] \\
				&= \E_{\vec{s}_{t - 1}, \vec{s}_t}[\vec{s}_{t - 1} \vec{s}_t^T \given \vec{y}_{1:t - 1}] - \E_{\vec{s}_{t - 1}}[\vec{s}_{t - 1} \given \vec{y}_{1:t - 1}] \big( \E_{\vec{s}_t}[\vec{s}_t \given \vec{y}_{1:t - 1}] \big)^T \\
				&\oversetfootnotemark{=} \E_{\vec{s}_{t - 1}}[\vec{s}_{t - 1} \vec{s}_{t - 1}^T \mat{A}^T \given \vec{y}_{1:t - 1}] - \hat{\vec{s}}_{t - 1 \subgiven t - 1} \hat{\vec{s}}_{t \subgiven t - 1}^T \\
				&= \E_{\vec{s}_{t - 1}}[\vec{s}_{t - 1} \vec{s}_{t - 1}^T \given \vec{y}_{1:t - 1}] \mat{A}^T - \hat{\vec{s}}_{t - 1 \subgiven t - 1} \hat{\vec{s}}_{t \subgiven t - 1}^T \\
				&= \mat{V}_{t - 1 \subgiven t - 1} \mat{A}^T - \hat{\vec{s}}_{t - 1 \subgiven t - 1} \hat{\vec{s}}_{t \subgiven t - 1}^T
		\end{align*}
		\footnotetext{Due to the independence and zero mean of the noise, it disappears in the expectation: \( \E_{\vec{s}_{t - 1}, \vec{s}_t}[\vec{s}_{t - 1} \vec{s}_t^T \given \vec{y}_{1:t - 1}] = \E_{\vec{s}_{t - 1}, \vec{w}_t}[\vec{s}_{t - 1} \big(\vec{s}_{t - 1}^T \mat{A}^T + \vec{w}_t^T\big) \given \vec{y}_{1:t - 1}] = \E_{\vec{s}_{t - 1}, \vec{w}_t}[\vec{s}_{t - 1} \vec{s}_{t - 1}^T \mat{A}^T + \vec{s}_{t - 1} \vec{w}_t^T \given \vec{y}_{1:t - 1}] = \E_{\vec{s}_{t - 1}, \vec{w}_t}[\vec{s}_{t - 1} \vec{s}_{t - 1}^T \mat{A}^T \given \vec{y}_{1:t - 1}] \)}
		This completes the derivation of the backward pass equations.
		
		For completeness, we summarize all of them here:
		\begin{align}
			\mat{V}_{t - 1, t \subgiven t - 1} & = \mat{V}_{t - 1 \subgiven t - 1} \mat{A}^T - \hat{\vec{s}}_{t - 1 \subgiven t - 1} \hat{\vec{s}}_{t \subgiven t - 1}^T  \nonumber \\
			\mat{J}_{t - 1}                    & = \mat{V}_{t - 1, t \subgiven t - 1} \mat{V}_{t \subgiven t - 1}^{-1}  \nonumber \\
			\hat{\vec{s}}_{t - 1 \subgiven T}  & = \hat{\vec{s}}_{t - 1 \subgiven t - 1} + \mat{J}_{t - 1} (\hat{\vec{s}}_{t \subgiven T} - \hat{\vec{s}}_{t \subgiven t - 1})  \label{eq:posteriorMean} \\
		...
		\begin{align*}
			\mat{P}_t &= \mat{V}_{t \subgiven T} + \hat{\vec{s}}_{t \subgiven T} \hat{\vec{s}}_{t \subgiven T} \\
			\mat{P}_{t, t - 1} &= \mat{J}_{t - 1} \mat{V}_{t \subgiven T} + \hat{\vec{s}}_{t \subgiven T} \hat{\vec{s}}_{t - 1 \subgiven T}
		\end{align*}
