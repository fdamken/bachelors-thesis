...
%			\label{subsubsec:polynomial}
%
%			\begin{itemize}
%				\item Experiment ID: \texttt{polynomial}
%			\end{itemize}
%
%			This is our second proof-of-concept and the first that actually proofs our concept of learning nonlinear dynamics with a linear embedding. For this environments we use a dynamical system with the state \( \vec{y} \coloneqq \begin{bmatrix} x_1 & x_2 \end{bmatrix}^T \) that is described by the polynomial dynamics
%			\begin{align*}
%				\dot{x}_1 &= \mu x_1 \\
%				\dot{x}_2 &= \lambda (x_2 - x_1^2)
%			\end{align*}
%			with parameters \( \mu \), \( \lambda \). We set \( \mu = -0.05 \) and \( \lambda = -1 \), for which the system is stable and does not exhibit exponential growth. The initial state is sampled from a Gaussian with mean \( \begin{bmatrix} 0.4 & 0.5 \end{bmatrix} \) and covariance \( \diag(0.1, 0.1) \). The system is integrated using the implicit Runge-Kutta Radau~IIA~\cite{guglielmiImplementingRadauIIA2001} method with an evaluation interval of \( h = 0.02 \) for \( T = 100 \) time steps where only the first \( T_\train = 50 \) steps are used for training and the remaining \(50\) are used for validation. The raw data is shown in~\autoref{fig:envPolynomial}.
%
%			We chose this as a proof-of-concept environment as it is one of the few systems where the Koopman operator and the Koopman eigenfunctions can be derived analytically~\cite{bruntonKoopmanInvariantSubspaces2016}. The eigenfunctions are then given as
%			\begin{align*}
%				\varphi_1(x_1, x_2) &= x_1 \\
%				\varphi_2(x_1, x_2) &= x_2 \\
%				\varphi_3(x_1, x_2) &= x_1^2
%			\end{align*}
%			with the finite-dimensional "Koopman matrix":
%			\begin{equation*}
%				\mat{K} =
%					\begin{bmatrix}
%						\mu & 0       & 0        \\
...
%					\end{bmatrix}
%			\end{equation*}
%			We therefore should be able to find an appropriate embedding and inverse observation functions to perfectly model this polynomial environment.
%
%			\begin{figure}
%				\centering
%				\begin{subfigure}{0.5\linewidth}
%					\centering
%					\includegraphics[width=\linewidth]{figures/experiments/environments/observations-polynomial-N0-D0.pdf}
%				\end{subfigure}%
%				~
%				\begin{subfigure}{0.5\linewidth}
%					\centering
%					\includegraphics[width=\linewidth]{figures/experiments/environments/observations-polynomial-N0-D1.pdf}
%				\end{subfigure}
%				\caption{Plot of the raw data used for training the proof-of-concept polynomial environment. The black dots represent the actual data points, all before the red prediction boundary are used for training, the rest for validation. The faint gray line emphasizes the connection between the data points and that they are actually generated from a dynamical system.}
%				\label{fig:envPolynomial}
%			\end{figure}
%		% end
	...
	\subsection{Gym Cartpole}
		...
		\subsubsection{Exemplary Evaluation: 14-Dimensional Latent}
			...
			\begin{figure}
				...
				\caption{The rollout plot in the observation space of the cartpole environment for \(k = 14\). The top left plot shows the cart position, the top right plot the cart velocity, the bottom left plot the pendulum angle and the bottom right plot the pendulum velocity. The black dots represent the true data of which the model used everything till the red prediction boundary to train on. The blue line is the rollout, starting from the learned initial value (marked with a green star). The orange dash-dotted line is the smoothed data. The dotted orange line then is the rollout starting from the last smoothed state, forming the "smoothed prediction". The shaded regions show the confidence, \ac{ie} two times the standard deviation. As the variances in the top plots are still very high, see~\autoref{fig:cartpoleRolloutL14Appendix} in~\autoref{app:remainingPlots} for the plot without the confidence.}
	...
	\subsection{Gym Double Pendulum}
		...
		\subsubsection{Exemplary Evaluation: 3-Dimensional Latent}
			...
			\begin{figure}
				...
				\caption{The rollout plot in the observation space of the double pendulum environment for \(k = 3\). The top row shows the cosine/sine of the displacement of the inner pendulum, the middle row shows the cosine/sine of the displacement of the outer pendulum and the bottom row shows the two angular velocity with the inner pendulum on the left and the outer pendulum on the right. The black dots represent the true data of which the model used everything till the red prediction boundary to train on. The blue line is the rollout, starting from the learned initial value (marked with a green star). The orange dash-dotted line is the smoothed data. The dotted orange line then is the rollout starting from the last smoothed state, forming the "smoothed prediction". The shaded regions show the confidence, \ac{ie} two times the standard deviation.}
