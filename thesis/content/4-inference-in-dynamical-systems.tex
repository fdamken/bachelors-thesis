...
\section{Cubature Rules and Filtering}
	...
	\subsection{The Unscented Transform}
		...
		\begin{equation*}
			\mat{\mathcal{X}} = \sqrt{n + \lambda}
				\begin{bmatrix}
					\vec{\mu} & \vec{\mu} \vec{1}^T + \sqrt{\mat{\Sigma}} & \vec{\mu} \vec{1}^T - \sqrt{\mat{\Sigma}}
				\end{bmatrix}
		\end{equation*}
		...
		\end{equation*}
		is a scaling parameter defined using the parameters of the \ac{ut} \( \alpha \) and \( \kappa \). The approximation for the transformed mean and covariance is then given as
		\begin{align*}
			\hat{\vec{\mu}} \coloneqq \E_{\vec{x} \sim \normal(\vec{\mu}, \mat{\Sigma})}\big[ \vec{f}(\vec{x}) \big] &\approx \sum_{i = 1}^{2n + 1} w_{i - 1}^{(m)} \vec{f}(\mat{\mathcal{X}}_i) \\
			\hat{\mat{\Sigma}} \coloneqq \Cov_{\vec{x} \sim \normal(\vec{\mu}, \mat{\Sigma})}\big[ \vec{f}(\vec{x}) \big] &\approx \sum_{i = 1}^{2n + 1} w_{i - 1}^{(c)} \big( \vec{f}(\mat{\mathcal{X}}_i) - \hat{\vec{\mu}} \big) \big( \mat{\mathcal{X}}_i - \hat{\vec{\mu}} \big)^T
		\end{align*}
	...
	\subsection{Cubature Kalman Filtering and Smoothing}
		...
		\begin{algorithm}  \DontPrintSemicolon
			\( \vec{s}_{0 \subgiven 0} \gets \vec{m}_0 \) \;
			\( \vec{P}_{0 \subgiven 0} \gets \mat{V}_0 \) \;
			\For{\( t = 1, 2, \rangedots, T \)}{
				\( \vec{\xi}_i \gets \sqrt{n} [\vec{1}]_i \) \;
				\textbf{Prediction:} \;
				\quad\eqmakebox[ckf][r]{\( \vec{x}_{i, t - 1 \subgiven t - 1} \gets \)} \( \sqrt{\mat{P}_{t - 1 \subgiven t - 1}} \vec{\xi}_i + \vec{s}_{t - 1 \subgiven t - 1} \) \;
				...
				\textbf{Correction:} \;
				\quad\eqmakebox[ckf][r]{\( \vec{x}_{i, t \subgiven t - 1} \gets \)} \( \sqrt{\mat{P}_{t \subgiven t - 1}} + \vec{s}_{t \subgiven t - 1} \) \;
		...
		\begin{algorithm}  \DontPrintSemicolon
			\For{\( t = T, T - 1, \rangedots, 1 \)}{
				\eqmakebox[crts][r]{\( \vec{\xi}_i \gets \)} \(\sqrt{n} [\vec{1}]_i \) \;
				\eqmakebox[crts][r]{\( \vec{x}_i \gets \)} \( \sqrt{\mat{P}_{t \subgiven t}} \vec{\xi}_i + \vec{s}_{t \subgiven t} \) \;
