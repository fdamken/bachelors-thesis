...
\section{Cubature Rules and Filtering}
	...
	\subsection{Cubature Kalman Filtering and Smoothing}
		...
		\begin{algorithm}  \DontPrintSemicolon
			...
			\eqmakebox[ckfinit][r]{\( \vec{s}_{0 \subgiven 0} \gets \)} \( \vec{m}_0 \) \;
			\eqmakebox[ckfinit][r]{\( \mat{P}_{0 \subgiven 0} \gets \)} \( \mat{V}_0 \) \;
			\For{\( t = 1, 2, \rangedots, T \)}{
				\( \vec{\xi}_i \gets \sqrt{n} [\vec{1}]_i \) \;
				\textbf{Prediction:} \;
				\quad\eqmakebox[ckf][r]{\( \vec{x}_{i, t - 1 \subgiven t - 1} \gets \)} \( \mat{P}_{t - 1 \subgiven t - 1}^{1/2} \vec{\xi}_i + \vec{s}_{t - 1 \subgiven t - 1} \) \;
				\quad\eqmakebox[ckf][r]{\( \vec{s}_{t \subgiven t - 1} \gets \)} \( \frac{1}{2n} \sum_{i = 1}^{2n} \vec{f}(\vec{x}_{i, t - 1 \subgiven t - 1}) \)  \label{algline:ckfStatePre} \;
				\quad\eqmakebox[ckf][r]{\( \mat{P}_{t \subgiven t - 1} \gets \)} \( \frac{1}{2n} \sum_{i = 1}^{2n} \vec{f}(\vec{x}_{i, t - 1 \subgiven t - 1}) \vec{f}^T(\vec{x}_{i, t - 1 \subgiven t - 1}) - \vec{s}_{t \subgiven t - 1} \vec{s}_{t \subgiven t - 1}^T + \mat{Q} \)  \label{algline:ckfCovPre} \;
				\textbf{Correction:} \;
				\quad\eqmakebox[ckf][r]{\( \vec{x}_{i, t \subgiven t - 1} \gets \)} \( \mat{P}_{t \subgiven t - 1}^{1/2} + \vec{s}_{t \subgiven t - 1} \) \;
				\quad\eqmakebox[ckf][r]{\( \vec{y}_{t \subgiven t - 1} \gets \)} \( \frac{1}{2n} \sum_{i = 1}^{2n} \vec{g}(\vec{x}_{i, t \subgiven t - 1}) \)  \label{algline:ckfInno} \;
				\quad\eqmakebox[ckf][r]{\( \mat{S}_t \gets \)} \( \frac{1}{2n} \sum_{i = 1}^{2n} \vec{g}(\vec{x}_{i, t \subgiven t - 1}) \vec{g}^T(\vec{x}_{i, t \subgiven t - 1}) - \vec{y}_{t \subgiven t - 1} \vec{y}_{t \subgiven t - 1}^T + \mat{R} \)  \label{algline:ckfResCov} \;
				\quad\eqmakebox[ckf][r]{\( \mat{J}_t \gets \)} \( \frac{1}{2n} \sum_{i = 1}^{2n} \vec{f}(\vec{x}_{i, t - 1 \subgiven t - 1}) \vec{g}^T(\vec{x}_{i, t \subgiven t - 1}) - \vec{s}_{t \subgiven t - 1} \vec{y}_{t \subgiven t - 1}^T \) \;
				\quad\eqmakebox[ckf][r]{\( \mat{K}_t \gets \)} \( \mat{J}_t \mat{S}_t^{-1} \)  \label{algline:ckfKalmanGain} \;
				\quad\eqmakebox[ckf][r]{\( \vec{s}_{t \subgiven t} \gets \)} \( \vec{s}_{t \subgiven t - 1} + \mat{K}_t (\vec{y}_t - \vec{y}_{t \subgiven t - 1}) \)  \label{algline:ckfStatePost} \;
				\quad\eqmakebox[ckf][r]{\( \mat{P}_{t \subgiven t} \gets \)} \( \mat{P}_{t \subgiven t - 1} + \mat{K}_t \mat{S}_t \mat{K}_t^T \)  \label{algline:ckfCovPost} \;
			}
		...
		\begin{algorithm}  \DontPrintSemicolon
			...
			\For{\( t = T - 1, T - 2, \rangedots, 1 \)}{
				...
				\eqmakebox[crts][r]{\( \vec{x}_i \gets \)} \( \mat{P}_{t \subgiven t}^{1/2} \vec{\xi}_i + \vec{s}_{t \subgiven t} \) \;
				\eqmakebox[crts][r]{\( \mat{D}_t \gets \)} \( \frac{1}{2n} \sum_{i = 1}^{2n} (\vec{x}_i - \vec{s}_{t \subgiven t}) \big( \vec{f}(\vec{x}_i) - \vec{s}_{t + 1 \subgiven t} \big)^T \) \;
				\eqmakebox[crts][r]{\( \mat{J}_t \gets \)} \( \mat{D}_t \mat{P}_{t + 1 \subgiven t}^{-1} \) \;
				\eqmakebox[crts][r]{\( \vec{s}_{t \subgiven T} \gets \)} \( \vec{s}_{t \subgiven t} + \mat{J}_t (\vec{s}_{t + 1 \subgiven T} - \vec{s}_{t + 1 \subgiven t}) \)  \label{algline:crtsState} \;
				\eqmakebox[crts][r]{\( \mat{P}_{t \subgiven T} \gets \)} \( \mat{P}_{t \subgiven t} + \mat{J}_t (\mat{P}_{t + 1 \subgiven T} - \mat{P}_{t + 1 \subgiven t}) \mat{C}_t^T \)  \label{algline:crtsCov} \;
			}
	...
	\subsection{Square-Root Filtering/Smoothing}
		...
		\begin{algorithm}  \DontPrintSemicolon
			...
			\eqmakebox[sqrtckfinit][r]{\( \vec{s}_{0 \subgiven 0} \gets \)} \( \vec{m}_0 \) \;
			\eqmakebox[sqrtckfinit][r]{\( \mat{P}_{0 \subgiven 0}^{1/2} \gets \)} \( \mat{V}_0^{1/2} \) \;
			\For{\( t = 1, 2, \rangedots, T \)}{
				\eqmakebox[sqrtckf][r]{\( \mat{\Lambda} \gets \)} \( \sqrt{n} \cdot \mat{P}_{t - 1 \subgiven t - 1}^{1/2} \) \;
				\eqmakebox[sqrtckf][r]{\( \mat{\mathcal{X}}_{t - 1 \subgiven t - 1} \gets \)} \( \begin{bmatrix} \vec{s}_{t - 1 \subgiven t - 1} \vec{1}^T + \mat{\Lambda} & \vec{s}_{t - 1 \subgiven t - 1} \vec{1}^T - \mat{\Lambda} \end{bmatrix} \) \;
				...
				\eqmakebox[sqrtckf][r]{\( \vec{s}_{t \subgiven t - 1} \gets \)} \( \frac{1}{2k} \sum_{i = 1}^{2k} \mat{\mathcal{X}}_{t \subgiven t - 1, i}^s \) \;
				\eqmakebox[sqrtckf][r]{\( \vec{y}_{t \subgiven t - 1} \gets \)} \( \frac{1}{2k} \sum_{i = 1}^{2k} \mat{\mathcal{X}}_{t \subgiven t - 1, i}^y \) \;
				\eqmakebox[sqrtckf][r]{\( \mat{\mathcal{Y}}_{t \subgiven t - 1, i}^s \gets \)} \( \big(\mat{\mathcal{X}}_{t \subgiven t - 1, i}^s - \vec{s}_{t \subgiven t - 1}\big)/\sqrt{2k} \) \;
				\eqmakebox[sqrtckf][r]{\( \mat{\mathcal{Y}}_{t \subgiven t - 1, i}^y \gets \)} \( \big(\mat{\mathcal{X}}_{t \subgiven t - 1, i}^y - \vec{y}_{t \subgiven t - 1}\big)/\sqrt{2k} \) \;
				Find \( \mat{S}_t^{1/2} \), \( \mat{J}_t \) and \( \mat{P}_{t \subgiven t}^{1/2} \) using the QR decomposition: \\
				\quad \(
					\begin{bmatrix}
					...
					\begin{bmatrix}
						...
						\mat{J}_t       & \mat{P}_{t \subgiven t}^{1/2} & \mat{O}
					\end{bmatrix}
				\) \;
				\eqmakebox[sqrtckf][r]{\( \mat{K}_t \gets \)} \( \mat{J}_t \mat{S}_t^{-1/2} \) \;
				\eqmakebox[sqrtckf][r]{\( \vec{s}_{t \subgiven t} \gets \)} \( \vec{s}_{t \subgiven t - 1} + \mat{K}_t (\vec{y}_t - \vec{y}_{t \subgiven t - 1}) \) \;
			}
		...
		\begin{algorithm}
			\KwData{All values from the Kalman filter.}
			\For{\( t = T - 1, T - 2, \rangedots, 1 \)}{
				\eqmakebox[sqrtcrts][r]{\( \mat{\mathcal{Y}}_{t \subgiven t, i}^s \gets \)} \( (\mat{\mathcal{X}}_{t \subgiven t, i} - \vec{s}_{t \subgiven t}) \) \;
				...
				\quad \(
					...
					=
					\begin{bmatrix}
						\mat{P}_{t + 1 \subgiven t}^{1/2} & \mat{O}   & \mat{O} \\
						\mat{D}_t                         & \mat{Z}_t & \mat{O}
					\end{bmatrix}
				\) \;
				\eqmakebox[sqrtcrts][r]{\( \mat{J}_t \gets \)} \( \mat{D}_t \mat{P}_{t + 1 \subgiven t}^{-1/2} \) \;
				\eqmakebox[sqrtcrts][r]{\( \vec{s}_{t \subgiven T} \gets \)} \( \vec{s}_{t \subgiven t} + \mat{J}_t (\vec{s}_{t + 1 \subgiven T} - \vec{s}_{t + 1 \subgiven t}) \) \;
				Find \( \mat{P}_{t \subgiven T}^{1/2} \) using the QR decomposition: \\
				\quad \(
					\begin{bmatrix}
						\mat{Z}_t & \mat{J}_t \mat{P}_{t + 1 \subgiven T}^{1/2}
					\end{bmatrix}
					\mat{\Theta}
					=
					\begin{bmatrix}
						\mat{P}_{t \subgiven T}^{1/2} & \mat{O}
					\end{bmatrix}
				\)
			}
