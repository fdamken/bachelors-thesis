...
\titlegraphic*{\includegraphics{figures/damped-pendulum-motion}}
...
\begin{document}
	...
	\begin{frame}{How to learn?}
		...
		\begin{itemize}
			...
			\item Measurement function \( \vec{g}_{\vec{\theta}}(\cdot) \) (i.e. the parameters \( \vec{\theta} \)). \\ Variational auto-encoder without an amortization network. % TODO: Introduce ELBO and compare.
		...
		\onslide<3->{
			\begin{itemize}
				...
				\item<4->[] \(\qquad\longrightarrow\quad\) Use backpropagation!
			\end{itemize}
		}
	\end{frame}

	\section{Results} % TODO: Introduce experiment with different latents.
		\subsection{The Pendulum} % TODO: Merge plots and do train/test timesteps; transpose plots.
			\begin{frame}{Results: Damped Inverted Pendulum \\ Dynamical System}
				...
				\begin{minipage}{0.65\textwidth}
					\begin{figure}
						\centering
						\onslide<2->{\includegraphics[width=\textwidth]{figures/results/pendulum/log-likelihood.pdf}}
					\end{figure}
				\end{minipage}
			\end{frame}

			\begin{frame}{Results: Damped Inverted Pendulum \\ Rollout/Prediction in Observation Space, 10 Latents} % TODO: Change if no. of latents changes.
				% TODO: Extend real data until end and use only the start for training.
				\vspace{-0.4cm}
				\begin{figure}
					\centering
			...
			\begin{frame}{Results: Damped Inverted Pendulum \\ Different Sizes of Latent Dimension}
				% TODO: Run experiment and real plot.
				\vspace{-0.4cm}
				\begin{figure}
					\centering
